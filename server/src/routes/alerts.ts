...
import { KiwiPreconditions, KiwiStandardResponsesExpress } from "@kiwiproject/kiwi-js";
...
router.get("/alert/counts", async (req: Request, res: Response) => {
  ...
  await runAsUser(req, res, async (currentUser: any, req: Request, res: Response) => {
    try {
      ...
      KiwiStandardResponsesExpress.standardGetResponseWithMessage(counts, "error alert count", res);
    } catch (error) {
      logger.error(error);
...
router.get("/alert/list", async (req: Request, res: Response) => {
  ...
  await runAsUser(req, res, async (currentUser: any, req: Request, res: Response) => {
    ...
    try {
      ...
      const kiwiPage = await alertService.findByUserPaged(currentUser, perPage, page, skip, sortDir);
      KiwiStandardResponsesExpress.standardGetResponseWithMessage(kiwiPage, "error get alert list", res);
    } catch (error) {
      logger.error(error);
  ...
  await runAsUser(req, res, async (currentUser: any, req: Request, res: Response) => {
    try {
      ...
      if (alert.userId !== currentUser.id) {
        ...
        KiwiStandardResponsesExpress.standardGetResponseWithMessage(alert, "error get alert", res);
      }
    } catch (error) {
      logger.error(error);
...
router.post("/alert/", async (req: Request, res: Response) => {
  ...
  */
  await runAsUser(req, res, async (currentUser: any, req: Request, res: Response) => {
    try {
      logger.info("POST /alert/:  user %s, alert title %s", currentUser.id, req.body.title);
      const alert: IAlert = {
      ...
      KiwiStandardResponsesExpress.standardPostResponse(`${config.basePath}/alert/${alertDoc.id}`, alertDoc, res);
    } catch (error) {
      logger.error(error);
  ...
  */
  await runAsUser(req, res, async (currentUser: any, req: Request, res: Response) => {
    try {
      ...
      const alertDoc = await alertService.updateAlert(req.query.alertId as string, updates);
