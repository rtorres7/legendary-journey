  ...
  await runAsUser(req, res, async (currentUser, req, res) => {
    ...
    try {
      const pageOfRecentProducts =
        await productService.findPageOfRecentProductsForUser(
          currentUser.id,
          page,
          perPage,
          skip,
...
router.get("/workspace/stats", async (req, res) => {
  /*
    #swagger.summary = "Retrieve stats on current user's products"
    #swagger.tags = ['Workspace']
    #swagger.responses[200] = {
      schema: {
        type: "object",
        properties: {
          total_views: {
            type: "integer",
            description: "The total number of views across all products."
          },
          unique_views: {
            type: "integer",
            description: "The total number of unique views across all products."
          },
        },
      },
    }
  ...
  await runAsUser(req, res, async (currentUser, req, res) => {
    try {
      const viewsData = await workspaceService.getUserProductViews(
        currentUser.id,
      );
      res.json({
        total_views: viewsData.totalViews,
        unique_views: viewsData.uniqueViews,
      });
    } catch (error) {
      KiwiStandardResponsesExpress.standardErrorResponse(
        500,
        `Unable to find user's products view data: ${error.message}`,
        res,
      );
    }
  });
