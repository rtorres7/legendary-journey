...
class MetadataService {
  ...
  async findIssuesForTopics(topicCodes) {
    ...
    return await lookups.issues.values.filter(issue => {
      return issue.topics.map(topic => topic.codes).flat().filter(code => topicCodes.includes(code)).length > 0;
    });
  }

  async findProducingOfficesFor(codes) {
    const lookups = await this.findAllLookups();
    return await this.findLookupsByCodes(lookups.producing_offices, codes);
  }

  async findProductType(code) {
    const lookups = await this.findAllLookups();
    return await lookups.product_types.values.filter(productType => productType.code === code)[0];
  }

  async findReportingTypeFor(productTypeId) {
    const lookups = await this.findAllLookups();
    const types = await lookups.reporting_types.values.filter(reportingType => reportingType.productTypes.includes(productTypeId));
  ...
  async findSubRegionsForCountries(countries) {
    ...
    return await lookups.subregions.values.filter(subregion => {
      return subregion.country_codes.filter(code => countries.includes(code)).length > 0;
    });
  }
  ...
  async findRegionsForSubRegions(subregions) {
    ...
    return await lookups.regions.values.filter(region => {
      return region.subregions.filter(code => subregions.includes(code)).length > 0;
    });
  }

  async findCoauthorsFor(codes) {
    const lookups = await this.findAllLookups();
    return await this.findLookupsByCodes(lookups.coauthors, codes);
  }

  async findCoordinatorsFor(codes) {
    const lookups = await this.findAllLookups();
    return await this.findLookupsByCodes(lookups.coordinators, codes);
  }
  ...
  async findNonStateActorsFor(codes) {
    const lookups = await this.findAllLookups();
    return await this.findLookupsByCodes(lookups.non_state_actors.values, codes);
  }
