...
function setupMongoose() {
  const mongoBaseUrl = config.mongodb.url;
  ...
  mongoose.connect(mongoBaseUrl, { useNewUrlParser: true }).catch((error) => {
...
function handleInitialError(error) {
  ...
  if (initialConnectionRetries < 10) {
    console.warn(
      `There was a problem initially connecting to Mongo. We will try again up to ${
        10 - initialConnectionRetries
      } more times`,
      error.message,
    );
    ...
    console.error(
      "Unable to make an initial connection to Mongo after 10 tries (5 seconds). The app will not be able to work correctly",
      error.message,
    );
  }
}
...
function handleMongooseErrors(error) {
  console.warn(
    "There was an error while accessing Mongo. Mongoose should retry automatically",
    error.message,
  );
}
...
function handleMongooseDisconnect() {
  console.warn(
    "The app has been disconnected from Mongo. Mongoose should reconnect automatically",
  );
}
