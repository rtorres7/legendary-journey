  ...
  return jest.fn().mockImplementation(() => {
    return {
      search: jest.fn().mockResolvedValue({
        totalCount: 1,
        results: [{ id: "1", productNumber: "really-cool-product" }],
      }),
    };
  });
});
...
jest.mock("../../src/services/product-service.js", () => {
  return jest.fn().mockImplementation(() => {
    return {
      findProductsForIds: jest
        .fn()
        .mockResolvedValue([
          { _id: 1, productNumber: "really-cool-product", views: 1 },
        ]),
    };
  });
});
...
describe("Workspace Service", () => {
  ...
  describe("findPageOfSavedProductsForUser", () => {
    it("should return a page of saved products for user", async () => {
      const products = await service.findPageOfSavedProductsForUser(
        1,
        "",
        10,
        1,
  ...
  describe("deleteSavedProduct", () => {
    it("should delete a saved product", async () => {
      ...
      const savedProduct = await models.SavedProduct.create({
        productId: "to-be-deleted",
        createdBy: 1,
      });

      await service.deleteSavedProduct("to-be-deleted", 1);

      const expected = await models.SavedProduct.findOne({
  ...
  describe("findPageOfCollectionsForUser", () => {
    it("should return a page of collection for user", async () => {
      const collections = await service.findPageOfCollectionsForUser(
        1,
        1,
        10,
        0,
      ...
      expect(collections.content.map((collection) => collection.name)).toEqual([
        "Sample Collection",
      ]);
  ...
  describe("createCollection", () => {
    it("should save a new collection", async () => {
      const savedCollection = await service.createCollection({
        name: "foo-collection",
      });
  ...
  describe("updateCollection", () => {
    it("should update the given collection", async () => {
      ...
      const updated = await service.updateCollection(original.id, {
        name: "baz",
        description: "what?",
      });
  ...
  describe("deleteCollection", () => {
    it("should delete the given collection", async () => {
      ...
      const expected = await models.Collection.findOne({
  ...
  describe("findSavedProductsInCollection", () => {
    it("should return list of saved products associated with given collection", async () => {
      ...
      const product = await models.SavedProduct.findOne({
        where: { productId: "WIReWIRe_sample_1" },
      });

      await collection.addSavedProduct(product);

      const products = await service.findSavedProductsInCollection(
        collection.id,
        "",
      );
  ...
  describe("addSavedProductToCollection", () => {
    it("should add a new saved product to a given collection", async () => {
      ...
      const savedCollection = await service.addSavedProductToCollection(
        collection.id,
        product.id,
      );
  ...
  describe("removeSavedProductFromCollection", () => {
    it("should remove a saved product from a given collection", async () => {
      ...
      const savedCollection = await service.removeSavedProductFromCollection(
        collection.id,
        product.id,
      );
