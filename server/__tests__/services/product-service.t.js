...
describe("ProductService", () => {
  ...
  describe("updateProduct", () => {
    ...
    it("should rollback the update of the saved product if the indexing update fails", async () => {
      ...
      }).rejects.toThrow("There was a problem updating product in index");
  ...
  describe("deleteProduct", () => {
    it("should remove the product from the database and the index", async () => {
      const original = await Article.create({
        productNumber: "product-to-delete",
      });
  ...
  describe("findFeaturesAndBriefs", () => {
    ...
    const createPublished = async (productTypeCode) => {
      ...
      */
      ...
      const issues = mapDiss(
        await metadataService.findIssuesForTopics(["THS"]),
      );
      const nonStateActors = mapDiss(
        await metadataService.findNonStateActorsFor(["EU"]),
      );
      const producingOffices = mapDiss(
        await metadataService.findProducingOfficesFor(["DNI"]),
      );
      const productType = mapDiss(
        await metadataService.findProductType(productTypeCode),
      );
      const topics = mapDiss(await metadataService.findTopicsFor(["TERR"]));
      const article = new Article({
        classification: "UNC",
        classificationXml:
          "<?xml version='1.0'?>UNCLASSIFIED<class:Classification>",
        countries,
        createdAt: dayjs().toDate(),
        datePublished: dayjs().format(),
        htmlBody: "Creation test",
    ...
    it("should not return deleted products", async () => {
      ...
      expect(results.briefs.map((p) => p.productNumber)).not.toContain(
        "product-to-delete",
      );
    });
    ...
    it("should return in briefs but not in featured", async () => {
      ...
      while (briefs.length > 0) {
        ...
        for (const result of results.briefs) {
          expect(
            briefs.findIndex((i) => i.productNumber === result.productNumber) >=
              0,
          );
  ...
  describe("findPageOfRecentProductsForUser", () => {
    it("should return a page of recent products", async () => {
      const recentProducts = await service.findPageOfRecentProductsForUser(
        1,
        1,
        10,
        0,
    ...
    it("should not return deleted products", async () => {
      ...
      expect(recentProducts.content.map((p) => p.productNumber)).not.toContain(
        "product-to-delete",
      );
    });
  ...
  describe("findPageOfRecentProductsForProducingOffice", () => {
    it("should return a page of recent products", async () => {
      const results = await service.findPageOfRecentProductsForProducingOffice(
        "AGRICULTURE",
        1,
        10,
        0,
    ...
    it("should not return deleted products", async () => {
      ...
      const recentProducts =
        await service.findPageOfRecentProductsForProducingOffice(
          "AGRICULTURE",
          1,
          10,
          0,
    ...
    function verifyRecentProducts(results) {
      ...
      expect(results.content.map((product) => product.productNumber)).toEqual([
        "WIReWIRe_sample_4",
      ]);
  ...
  describe("findPageOfProductsForUser", () => {
    it("should return a page of all products", async () => {
      const products = await service.findPageOfProductsForUser(
        1,
        1,
        10,
        0,
    ...
    it("should not return deleted products", async () => {
      ...
      expect(products.content.map((p) => p.productNumber)).not.toContain(
        "product-to-delete",
      );
    });
