...
describe("ProductSearchService", () => {
  ...
  beforeAll(async () => {
    ...
    await ElasticSearchExtension.createIndex(
      "products",
      constant.indices[0].mappings,
      [
        {
  ...
  describe("search", () => {
    ...
    it("should allow filtering on producingOffices with multiple producingOffices using OR", async () => {
      const result = await service.search("", 10, 1, "desc", {
        producing_offices: ["ANCESTRY", "EDUCATION"],
      });
  ...
  describe("create", () => {
    it("should index the given product", async () => {
      ...
      expect(
        (await client.get({ index: "products", id: product.id }))._source,
  ...
  describe("delete", () => {
    it("should delete the given product from the index", async () => {
      ...
      expect(
        await client.exists({ index: "products", id: product.id }),
      ).toBeFalsy();
    });
  });
  ...
  describe("indexAttachment", () => {
    it("should index given attachment", async () => {
      const attachment = fs.readFileSync(
        path.resolve(__dirname, "../__utils__/pdfToIndex.pdf"),
      );
      const attachmentString = attachment.toString("base64");

      await service.indexAttachment(
        "64709619aa530082dd5cc40e",
        "12345",
        attachmentString,
      );

      const esRecord = await client.get({
        index: "products",
        id: "64709619aa530082dd5cc40e",
      });
  ...
  describe("removeIndexedAttachment", () => {
    it("should remove an indexed attachment", async () => {
      ...
      const esRecord = await client.get({
        index: "products",
        id: "64709619aa530082dd5cc416",
      });
      expect(esRecord._source.pdfVersion.content).toEqual("test");

      await service.removeIndexedAttachment(
        "64709619aa530082dd5cc416",
        "12345",
      );

      const esRecord2 = await client.get({
        index: "products",
        id: "64709619aa530082dd5cc416",
      });
