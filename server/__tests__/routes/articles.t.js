...
const { loadArticlesIntoMongo, loadMetadata, loadElasticSearch } = require("../__utils__/dataLoader");
...
describe('Article Routes', () => {
  ...
  beforeAll(async () => {
    ...
    const mongoUrl = `mongodb://${mongoContainer.getHost()}:${mongoContainer.getMappedPort(27017)}/mxms`
  ...
  beforeEach(async () => {
    await mongoose.connect(`mongodb://${mongoContainer.getHost()}:${mongoContainer.getMappedPort(27017)}/mxms`, {
  ...
  describe('POST /articles/processDocument', () => {
    ...
    it('should send update document when document_action is save', async () => {
      const original = await Article.findOne({productNumber: 'WIReWIRe_sample_1'});
      const postData = {
      ...
      return request(app)
        ...
        .then(async (res) => {
          ...
          const updated = await Article.findOne({productNumber: 'WIReWIRe_sample_1'});
          expect(updated.classificationXml).toBe('S');

          expect((await esClient.get({ index: "products", id: original.id }))._source.classificationXml).toEqual('S');
        });
    });
  ...
  describe('POST /articles', () => {
    it('should create the given article', () => {
      const postData = { document_action: 'save', classification: 'S', date_published: new Date()  };
      ...
      return request(app)
        ...
        .then(async (res) => {
          ...
          expect((await esClient.exists({ index: "products", id: res.body.article.id }))).toEqual(true);
        });
    });
  });
  ...
  describe('GET /articles/:id/edit', () => {
    it('should return the requested article for editing', async () => {
      const articleToBeFound = await Article.findById('64709619aa530082dd5cc416');
      return request(app)
  ...
  describe('PUT /articles/:id', () => {
    it('should update document', async () => {
      const original = await Article.findOne({productNumber: 'WIReWIRe_sample_1'});
      const postData = { document_action: 'save', classification: 'S', id: original.id, date_published: original.datePublished, doc_num: original.productNumber  };
