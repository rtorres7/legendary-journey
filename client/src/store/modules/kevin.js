  ...
  actions: {
    ...
    saveDraft({ state, commit }, form) {
      ...
      if (process.env.NODE_ENV === 'low') {
        ...
        axios.post("/articles/processDocument", {
    ...
    publishArticle({ state, commit }, form) {
      ...
      if (process.env.NODE_ENV === 'low') {
        ...
        axios.post("/articles/processDocument", {
          ...
          router.push({ name: "article", params: { doc_num: route.params.doc_num } })
          // } else {
    ...
    getDocumentData({ state, commit }, article) {
      ...
      if (process.env.NODE_ENV === 'low') {
        ...
        let url = "/wires/" + article.date + "/articles/" + article.id + "/getDocumentData";
        axios.get(url, {
          date: article.date,
          id: article.id,
        }).then(response => {
